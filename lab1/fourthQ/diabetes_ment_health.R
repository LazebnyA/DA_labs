...
# ggplot(diabetes_ment_hlth, aes(x = Diabetes, y = MentHlth, color = Diabetes)) +
#   geom_point(aes(size = Count)) +  # Указываем размер точек в зависимости от количества элементов
...
diabetes_true <- diabetes_data %>%
  filter(Diabetes == TRUE)

num_intervals <- ceiling(1 + log2(length(diabetes_true$MentHlth)))
min_ment_health_val <- min(diabetes_true$MentHlth)
max_ment_health_val <- max(diabetes_true$MentHlth)
interval_width <- (max_ment_health_val - min_ment_health_val) / num_intervals

ggplot(diabetes_true, aes(x = MentHlth)) +
  ...
  labs(title = "MentHlth Distribution among Diabetics",
       x = "MentHlth",
       y = "Frequency / Density") +
  theme_minimal()


ggsave("lab1/fourthQ/img/diabetes_ment_health_hist.jpg", plot = last_plot(), width = 8, height = 6, dpi = 300)
