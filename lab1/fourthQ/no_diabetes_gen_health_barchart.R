...
diabetes_false <- diabetes_data %>%
  filter(Diabetes == FALSE) %>%
  group_by(GenHlth) %>%
  summarise(count = n())

diabetes_gen_hlth_barchart <- ggplot(diabetes_false, aes(x = GenHlth, y = count, fill = GenHlth)) + 
...
ggsave("lab1/fourthQ/img/no_diabetes_gen_health_barchart.jpg", plot = diabetes_gen_hlth_barchart, width = 8, height = 6, dpi = 300)
