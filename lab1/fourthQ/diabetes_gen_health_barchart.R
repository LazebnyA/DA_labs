...
diabetes_true_gen_hlth <- diabetes_data %>%
  filter(Diabetes == TRUE) %>%
  group_by(GenHlth) %>%
  summarise(count = n())

diabetes_gen_hlth_barchart <- ggplot(diabetes_true_gen_hlth, aes(x = GenHlth, y = count, fill = GenHlth)) + 
  geom_bar(stat = "identity", position = "dodge", width = 1) + 
  ggtitle("Gen health distribution among people sick on diabetes.") +
  xlab("Gen. Health category") +
  ylab("Count") +
  scale_y_continuous(labels = scales::comma) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8)) 

diabetes_gen_hlth_barchart

ggsave("lab1/fourthQ/img/diabetes_gen_health_barchart.jpg", plot = chol_check_age_barchart, width = 8, height = 6, dpi = 300)
