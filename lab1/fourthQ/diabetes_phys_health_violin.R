...
count_phys_health <- diabetes_data %>%
  filter(Diabetes == TRUE) %>%
  group_by(PhysHlth) %>%
  summarize(count = n())

# Теперь мы можем использовать размер точек в зависимости от количества элементов
ggplot(diabetes_data, aes(x = Diabetes, y = PhysHlth)) +
  geom_violin() +  
  labs(title = "Scatter plot of Phys. Health Rating by Diabetes",
       x = "Diabetes",
       y = "Phys. Health Rating") +
  theme_minimal()


ggsave("lab1/fourthQ/img/diabetes_phys_health_violin.jpg", plot = last_plot(), width = 8, height = 6, dpi = 300)
