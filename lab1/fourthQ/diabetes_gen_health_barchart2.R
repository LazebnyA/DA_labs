...
count_by_gen_hlth_diab_true <- diabetes_data %>%
  filter(Diabetes == TRUE) %>%
  group_by(GenHlth) %>%
  summarise(count = n())

count_by_gen_hlth_diab_false <- diabetes_data %>%
  filter(Diabetes == FALSE) %>%
  group_by(GenHlth) %>%
  summarise(count = n())

combined_data <- rbind(
  count_by_gen_hlth_diab_false %>% mutate(Diabetes = "FALSE"),
  count_by_gen_hlth_diab_true %>% mutate(Diabetes = "TRUE")
)

combined_plot <- ggplot(combined_data, aes(x = GenHlth, y = count, fill = Diabetes)) +
  geom_col(position = "identity", width = 0.95) +
  ggtitle("Gen Hlth among diabetes true and false. Порівняння розподілів") +
  xlab("Gen Hlth category") +
  ylab("Count") +
...
ggsave("lab1/fourthQ/img/diabetes_gen_health_barchart2.jpg", plot = combined_plot, width = 8, height = 6, dpi = 300)
