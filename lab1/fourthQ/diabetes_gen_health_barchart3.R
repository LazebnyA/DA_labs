...
percent_diab_true <- diabetes_data %>%
  group_by(GenHlth) %>%
  summarise(percent_diab_true = sum(Diabetes == TRUE) / n() * 100)

total_percent_diab_true <- sum(percent_diab_true$percent_diab_true)


diabetes_gen_health_barchart3 <- ggplot(percent_diab_true, aes(x = GenHlth, y = percent_diab_true)) +
  geom_col(fill = "skyblue", width = 0.95) +
  ggtitle("Percentage of people with Diabetes == TRUE by GenHlth category") +
  xlab("GenHlth category") +
  ylab("Percentage") +
  scale_y_continuous(labels = scales::comma) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8))

print(diabetes_gen_health_barchart3)

ggsave("lab1/fourthQ/img/diabetes_gen_health_barchart3.png", plot = diabetes_gen_health_barchart3, width = 8, height = 6, dpi = 300)
